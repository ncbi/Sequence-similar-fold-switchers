...
path_clustalo = '/Users/lauren.lea.porter/Downloads/script/clustalo' # path to clustalo binary
...
def calculate_stats(df, np):
	...
	for i in range(len(df)):
		...
		for j in range(len(df)):
			if i == j:
				continue
			mis = count_mismatches(df.iloc[i]['jpred_aln'], \
				df.iloc[j]['jpred_aln'], np)
...
if __name__ == '__main__':
	...
	parser.add_argument('-np', type=int, metavar='N_PRED', \
		default=-50, help='number of residues to use for prediction')
